...
\begin{titlepage}
  ...
  \vspace{0.3cm}
  {\large Gabriel Caixeta Romero\par}
...
O resultado aparece em duas abas: Transmissor e Receptor. Cada aba mostra os bits e sinais de cada camada, o texto recuperado e metricas como potencias, relacao sinal-ruido (SNR) e contagem de erros.
...
O simulador foi escrito em Python 3. A interface grafica usa PyGObject (GTK 3) e a biblioteca matplotlib serve apenas para plotar os sinais.
...
\begin{table}[H]
  ...
  \begin{tabular}{p{3.5cm} p{6.5cm} p{3cm}}
    \toprule
    \textbf{Arquivo} & \textbf{Papel} & \textbf{Requisito} \\
    \midrule
    \texttt{simulador.py} & Rotina principal: orquestra TX $\rightarrow$ meio $\rightarrow$ RX em \textit{threads} & Simulador \\
    \texttt{camada\_fisica.py} & Modulação digital (banda-base) e por portadora & CamadaFisica \\
    \texttt{camada\_enlace.py} & Enquadramento, detecção e correção de erros & CamadaEnlace \\
    \texttt{interface\_gui.py} & GUI em GTK 3 (entrada, configuração, gráficos) & InterfaceGUI \\
    \texttt{camada\_aplicacao.py} & Texto $\leftrightarrow$ bits (codificador/decodificador) & apoio \\
    \texttt{meio\_comunicacao.py} & Canal: sinal em Volts + ruído gaussiano $n(x,\sigma)$ & apoio (meio) \\
    \texttt{testes.py} & Suíte de testes de ida-e-volta (TX$\rightarrow$RX sem ruído == entrada) & validação \\
    \bottomrule
  \end{tabular}
\end{table}

\subsection{Diagrama de Pilha (TX $\rightarrow$ MEIO $\rightarrow$ RX)}

A figura a seguir ilustra o caminho da mensagem.

No transmissor, o texto desce pelas camadas de aplicacao, enlace e fisica ate virar um sinal eletrico. No meio, o ruido gaussiano e somado amostra a amostra. No receptor, o sinal sobe de volta pelas camadas ate ser reconstruido como texto. A comunicação entre as \textit{threads} ocorre por duas filas: TX \texttt{--fila\_tx-->} [canal soma ruído] \texttt{--fila\_rx-->} RX.

% Diagrama em ASCII (alternativa: substituir por imagem)
\begin{lstlisting}
  THREAD TX                       MEIO                    THREAD RX
  ---------                       ----                    ---------
  texto                                                       texto
    | aplicacao (texto->bits)                 (bits->texto) aplicacao
  bits                                                        bits
    | enlace (EDC/Hamming + enquadramento)  (inverso) enlace |
  quadros (bits)                                   quadros (bits)
    | fisica (banda-base -> portadora)     (demod/decod) fisica
  sinal (V) --> [ + ruido gaussiano n(x, sigma) ] --> sinal (V)
\end{lstlisting}

\figplaceholder{Arquivo a inserir no Overleaf: \texttt{diagrama\_pilha.png} --- diagrama de pilha TX $\to$ MEIO $\to$ RX, mostrando as camadas Aplicacao, Enlace e Fisica no transmissor e no receptor, com o canal somando ruido gaussiano.}
...
\figplaceholder{Arquivo a inserir no Overleaf: \texttt{sinais\_banda\_base.png} --- formas de onda das modulacoes banda-base (NRZ-Polar, Manchester e Bipolar) para a mesma sequencia de bits.}
...
Funcoes auxiliares como \texttt{\_onda}, \texttt{\_correlacionar}, \texttt{\_pad} e \texttt{\_bits\_do\_ponto\_mais\_proximo} sao reaproveitadas entre os esquemas.

\figplaceholder{Arquivo a inserir no Overleaf: \texttt{constelacoes\_portadora.png} --- constelacoes QPSK e 16-QAM e formas de onda ASK e FSK.}
...
\figplaceholder{Arquivo a inserir no Overleaf: \texttt{hamming\_8\_4.png} --- bloco Hamming(8,4) com as posicoes dos bits de paridade e o calculo da sindrome.}
...
O painel direito tem duas abas, Transmissor e Receptor. Cada uma mostra os bits de cada camada, o texto recuperado, um relatorio por quadro (EDC OK ou com erro, bits corrigidos e erro duplo) e os graficos dos sinais, com matplotlib embutido no GTK. A separacao entre TX e RX fica explicita, atendendo ao enunciado.

\figplaceholder{Arquivo a inserir no Overleaf: \texttt{interface\_gtk.png} --- interface grafica em GTK com o painel de configuracao e as abas Transmissor e Receptor.}
...
\figplaceholder{Arquivo a inserir no Overleaf: \texttt{sinal\_com\_ruido.png} --- sinal recebido com ruido gaussiano sobreposto ao sinal transmitido.}
...
\figplaceholder{Arquivo a inserir no Overleaf: \texttt{testes\_terminal.png} --- saida do terminal com a mensagem ``TODOS OS TESTES PASSARAM''.}

%==================== MEMBROS ====================
\section{Membros e Atividades Desenvolvidas}

O trabalho foi dividido entre os tres integrantes do grupo, conforme as principais frentes do projeto.

\textbf{Gabriel Caixeta Romero} ficou responsavel pela camada de enlace: as tres tecnicas de enquadramento, os codigos de deteccao (paridade, checksum e CRC-32) e o Hamming, alem da suite de testes de ida-e-volta.

\textbf{Gustavo Henrique Andrade Cavalcanti} desenvolveu a camada fisica: as modulacoes banda-base (NRZ-Polar, Manchester e Bipolar) e por portadora (ASK, FSK, QPSK e 16-QAM), alem do modulo do meio de comunicacao com ruido gaussiano.

\textbf{Fernando Augusto Hortencio} cuidou da interface grafica em GTK, da integracao do simulador com a comunicacao entre as \textit{threads} de transmissor e receptor por filas, e da redacao do relatorio.

Apesar dessa divisao, houve colaboracao continua entre os membros nas etapas de integracao, testes e revisao.
